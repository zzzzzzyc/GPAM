...
if [ ${DATASET} == "pheme" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/domain_specific/fake_news/evaluate_pheme_weibo.py --checkpoint ${CHECKPOINT} --datasets pheme "${ARGS[@]:2}"
fi
...
if [ ${DATASET} == "weibo" ]; then
    torchrun \
      ...
      --nproc_per_node=${GPUS} \
      --master_port=${MASTER_PORT} \
      eval/domain_specific/fake_news/evaluate_pheme_weibo.py --checkpoint ${CHECKPOINT} --datasets weibo "${ARGS[@]:2}"
fi
