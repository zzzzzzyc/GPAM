...
from internvl.train.constants import (BOX_END_TOKEN, BOX_START_TOKEN,
                                      ...
                                      GRAPH_START_TOKEN, GRAPH_END_TOKEN, GRAPH_CONTEXT_TOKEN,
...
class ModelArguments:
    ...
    graph_encoder_path: Optional[str] = field(
        default=None,
        metadata={'help': 'Path to a pretrained model (local or from huggingface.co/models).'}
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        logger.info(f'[Dataset] num_graph_token: {num_graph_token}')
                ...
                if 'length' in data_item:
                    ...
                    if str_length not in self.conv2length:
                        ...
                        self.conv2length[str_length] = token_length + num_image_token * (
                                    max_dynamic_patch + use_thumbnail) + num_graph_token
                    else:
    ...
    def get_graph_path(self, graph_path):
        graph_path = os.path.join(self.root, graph_path)
        return graph_path
    ...
    def multi_modal_get_item(self, data_item):
        ...
        graph = Data(
            ...
            y=torch.tensor([0]),
            no_root_edge_index=torch.empty((2, 0), dtype=torch.long),
            centrality=torch.zeros((1,))
        )
        ...
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  self.tokenizer, self.num_graph_token, [self.num_image_token * num_patches], image_text=True,
        ...
        ret = dict(
            ...
            graph_flags=torch.tensor([0], dtype=torch.long)
        )
        return ret

    def multi_modal_graph_get_item(self, data_item):
        # Build transformation function
        transform = self.get_transform()
        ...
        # Merge the image path
        image_path = self.get_image_path(data_item['image'])
        graph_path = self.get_graph_path(data_item['graph'])
        ...
        # Preprocess the conversations and generate the return dictionary
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  self.tokenizer, self.num_graph_token,[self.num_image_token * num_patches],three_modal=True,
                                  group_by_length=self.group_by_length,
                                  use_packed_ds=self.use_packed_ds, ds_name=self.ds_name)
        ...
        ret = dict(
            ...
            pixel_values=pixel_values,
            image_flags=torch.tensor([1] * num_patches, dtype=torch.long),
            graph_flags=torch.tensor([1], dtype=torch.long)
        )
        return ret
    ...
    def pure_text_graph_get_item(self, data_item):
        # Build transformation function
        transform = self.get_transform()
        ...
        # Preprocess the conversations and generate the return dictionary
        ret = preprocess_function(self.template_name, [deepcopy(data_item['conversations'])],
                                  self.tokenizer, self.num_graph_token,[self.num_image_token * num_patches], graph_text=True,
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                if 'image' in data_item and len(data_item['image']) != 0 and 'graph' not in data_item:
                ...
                elif 'image' in data_item and len(data_item['image']) != 0 and 'graph' in data_item and len(data_item['graph']) != 0:
                    ret = self.multi_modal_graph_get_item(data_item)
                elif 'graph' in data_item and len(data_item['graph']) != 0 and 'image' not in data_item:
                    ret = self.pure_text_graph_get_item(data_item)
                else:
...
def build_datasets(
    ...
    for ds_idx, ds_name in enumerate(ds_collections.keys()):
        ...
        dataset = LazySupervisedDataset(
            ...
            tcs_loader,
            ds_name=ds_name,
            num_graph_token=data_args.num_graph_token,
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLChatModel.from_pretrained(
            model_args.model_name_or_path, torch_dtype=torch.bfloat16, config=config)
        if model_args.graph_encoder_path is not None:
            logger.info(f'Loading graph_model weights from {model_args.graph_encoder_path}')
            state_dict = torch.load(model_args.graph_encoder_path, map_location='cpu')
            model.graph_model.GT.load_state_dict(state_dict, strict=True)
            model.graph_model.to(dtype=torch.bfloat16)
        else:
            logger.warning('No graph_encoder_path provided, graph_model will be randomly initialized.')
    else:
    ...
    model.graph_context_token_id = graph_context_token_id
    ...
    if model_args.freeze_graph_encoder:
        # model.vision_model = model.vision_model.eval()
        # _freeze_params(model.graph_model)
        for n, p in model.graph_model.named_parameters():
            if n.split('.')[0] == 'GT':
    ...
    if model_args.freeze_mlp_graph:
