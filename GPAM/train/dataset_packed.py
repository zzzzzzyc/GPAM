...
class PackedDataset(IterableDataset):
    ...
    def update_buffer(self, buffer, new_sample):
        ...
        for k in buffer:
            if isinstance(buffer[k], list) and isinstance(buffer[k][0], (Data, Batch)):
                buffer[k].append(new_sample[k])
            elif isinstance(buffer[k], torch.Tensor):
                buffer[k] = torch.cat([buffer[k], new_sample[k]], dim=0)
            elif isinstance(buffer[k], list):
                buffer[k].extend(new_sample[k] if isinstance(new_sample[k], list) else [new_sample[k]])
            else:
                raise TypeError(f"Unsupported buffer type for key '{k}': {type(buffer[k])}")
        return buffer
    ...
    def postprocess_buffer(self, buffer, custom_infos=None):
        for k, v in buffer.items():
            if isinstance(v, list) and len(v) > 0 and isinstance(v[0], (Data, Batch)):
                buffer[k] = Batch.from_data_list(v)
