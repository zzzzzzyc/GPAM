...
def concat_pad_data_collator(features, max_item_length=None, pad_id=0):
    ...
    if 'graph' in first and isinstance(first['graph'], (Data, Batch)):
        graph_list = [f['graph'] for f in features]
    ...
    # Again, we will use the first element to figure out which key/values are not None for this model.
    for k, v in first.items():
        if k in ('label', 'label_ids', 'pixel_values', 'image_flags', 'graph', 'graph_flags'):
            continue
    ...
    for k in ('pixel_values', 'image_flags', 'graph_flags'):
        if k in first:
